...
const ITEMS = [
  ...
  { mub:{m:"المدينةُ الكبيرةُ",a:"المدينةَ الكبيرةَ",j:"المدينةِ الكبيرةِ"}, khb:{m:"هادئةٌ",a:"هادئةً",j:"هادئةٍ"} },
...
function renderLive() {
  ...
  // نعرض المبتدأ والخبر مع قابلية التحديد بالألوان
  const mHTML = m.split(" ").map(w => 
    `<span class='word ${state.mSel && state.mWord === w ? "blue" : ""}' data-part='m'>${w}</span>`
  ).join(" ");

  const kHTML = k.split(" ").map(w => 
    `<span class='word ${state.kSel && state.kWord === w ? "red" : ""}' data-part='k'>${w}</span>`
  ).join(" ");
